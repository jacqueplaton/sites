/* ==========================================================================
   LEAD AGENCY — JavaScript da interface.
...
const ROTULOS_STATUS = [
  'NOVO', 'QUALIFICADO', 'SITE_CRIADO', 'ABORDAR', 'ABORDADO', 'RESPONDEU',
  'INTERESSADO', 'REUNIAO', 'PROPOSTA', 'NEGOCIACAO', 'FECHADO', 'PERDIDO',
  'NAO_INTERESSADO',
];
...
async function iniciarLeads() {
          ...
          <tr>
            ...
            <td>
              <select data-status-de="${lead.id}">
                ${ROTULOS_STATUS.map((s) => `<option value="${s}" ${s === lead.status ? 'selected' : ''}>${s.replaceAll('_', ' ')}</option>`).join('')}
              </select>
            </td>
  ...
  $('#tabela-leads').addEventListener('change', async (evento) => {
    ...
    try {
      ...
      avisar(`Lead #${id} agora está em ${evento.target.value.replaceAll('_', ' ')}.`);
    } catch (erro) {
      avisar(erro.message, true);
    }
  });
      ...
      <section class="painel">
        <div class="audit-cabecalho">
          ...
          <div>
            ...
            <div>${etiquetaFaixa(lead.faixa)} <span class="pastilha">${escapar(lead.status.replaceAll('_', ' '))}</span> ${pastilhaSite(lead)}</div>
          </div>
        </div>
...
async function iniciarCrm() {
  async function carregar() {
    ...
    $('#kanban').innerHTML = ROTULOS_STATUS.map((status) => `
      <div class="coluna">
        <h3>${status.replaceAll('_', ' ')} <span>${porStatus[status].length}</span></h3>
  ...
  $('#kanban').addEventListener('change', async (evento) => {
    ...
    try {
      ...
      await carregar();
      avisar(`Lead #${id} movido para ${evento.target.value.replaceAll('_', ' ')}.`);
    } catch (erro) {
      avisar(erro.message, true);
    }
  });
