...
from app.core.config import configurar_logs, settings
...
def main() -> None:
    parser = argparse.ArgumentParser(description="LEAD AGENCY")
    ...
    logging.getLogger("lead_agency").info(
        "LEAD AGENCY em http://%s:%s (banco: %s)", args.host, args.port, settings.db_path
    )
    if args.reload:
