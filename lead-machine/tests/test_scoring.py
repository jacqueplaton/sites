...
from app.config import carregar_config, salvar_config
from app.models import SituacaoSite, StatusWebsite
from app.scoring import calcular_score
...
def test_lead_ideal_cai_na_faixa_hot():
    ...
    # + 10 alto ticket + 5 estabelecida + 5 presença digital fraca = 95
...
def test_site_profissional_desconta():
    ...
    # perde os 30 do "sem site", leva -20 e deixa de ter presença digital fraca
    assert resultado.score == calcular_score(LEAD_HOT).score - 30 - 20 - 5
...
def test_faixas_cobrem_todos_os_limites():
    ...
    from app.scoring import faixa_do_score
    config = carregar_config()
...
def test_presenca_digital_fraca_e_poucos_sinais():
    ...
    assert {"presenca_digital_fraca", "poucos_sinais_operacao", "aparentemente_inativa"} <= aplicadas
