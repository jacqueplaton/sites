...
O fluxo completo previsto é **busca → coleta → dedupe → detecção de site →
qualificação/score → análise IA → mensagem → CRM → follow-up → métricas**.
Esta entrega é a **Fase 1**; o que já roda e o que ainda não roda está listado
em [Status por fase](#status-por-fase).
...
Outras opções: `python main.py --reload` (recarrega ao salvar),
`--port 9000`, `--seed --forcar`.
...
```
lead-machine/
├── main.py                  entrada (servidor / --seed)
├── requirements.txt
├── .env.example             variáveis de ambiente (copie para .env)
├── Dockerfile · docker-compose.yml
├── app/
│   ├── app.py               fábrica do FastAPI, logs, tratamento de erro
│   ├── config.py            .env + data/config.json (pesos, limiares, nichos)
│   ├── database.py          engine SQLite + sessão
│   ├── models.py            tabela leads e os enums de status
│   ├── schemas.py           entrada e saída da API (pydantic)
│   ├── crud.py              criação, filtros, status, métricas
│   ├── dedupe.py            chave composta e similaridade de nomes
│   ├── website_detector.py  detect_missing_website()
│   ├── http_client.py       timeout, retry, rate limit por host, cache
...
**Status:** `NOVO`, `QUALIFICADO`, `ABORDADO`, `RESPONDEU`, `INTERESSADO`,
`REUNIAO`, `PROPOSTA`, `NEGOCIACAO`, `FECHADO`, `PERDIDO`, `NAO_INTERESSADO`.
...
| Empresa estabelecida (50+ avaliações) | +5 |
| Presença digital fraca (≤1 canal ativo) | +5 |
| Já tem site próprio no ar | −20 |
| Sem qualquer avaliação registrada | −10 |
| Poucos sinais de operação (<2) | −10 |
...
deixou configurável: *empresa estabelecida* = 50+ avaliações; *presença digital
fraca* = no máximo 1 canal ativo entre site confirmado, Instagram e Facebook;
*aparentemente inativa* = nenhuma avaliação e nenhuma nota; *poucos sinais de
operação* = menos de 2 entre telefone, endereço, horário e avaliações.
...
| `LM_USER_AGENT` | `LeadMachine/1.0 …` | identificação nas requisições |
